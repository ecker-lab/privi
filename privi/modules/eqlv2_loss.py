...
class EQLv2(nn.Module):
    def __init__(self,
        ...
        if class_weight is not None:
            assert class_weight.ndim == 1 and class_weight.shape[0] == num_classes, \
                f"class_weight must be a 1D tensor of shape ({num_classes},) "
    ...
    def forward(self,
        ...
        cls_loss = F.binary_cross_entropy_with_logits(cls_score, target.to(dtype=cls_score.dtype),
                                                      reduction='none')
        cls_loss = torch.sum(cls_loss * weight, dim=-1) # shape (N,)
