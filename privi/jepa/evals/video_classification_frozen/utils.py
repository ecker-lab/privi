...
import privi.jepa.src.datasets.utils.video.transforms as video_transforms
import privi.jepa.src.datasets.utils.video.volume_transforms as volume_transforms

from privi.jepa.src.datasets.utils.video.randerase import RandomErasing

from privi.jepa.src.models.utils.pos_embs import get_1d_sincos_pos_embed
...
def make_transforms(
    ...
    if not training and num_views_per_clip > 1:
        ...
        # If not training, this does center cropping and otherwise the same as EvalVideoTransform
...
class VideoTransform(object):
    ...
    def __init__(
        ...
        Args:
            training (bool): whether to use training augmentations.
            random_horizontal_flip (bool): whether to apply random horizontal flip.
            random_resize_aspect_ratio (tuple): range of aspect ratios for random resized crop.
            random_resize_scale (tuple): range of scales for random resized crop.
            reprob (float): probability of applying random erasing (size: 0.02 to 1/3 of the image area).
            auto_augment (bool): whether to apply auto-augment transformations, i.e. 4 random 
                 augmentations per image, e.g. rotate, color jitter, shear, etc.
            motion_shift (bool): whether to apply motion shift in spatial cropping, i.e. sample two
                 bounding boxes, one for start and one for end of the clip. Linear interpolation 
                 between the two boxes is applied.
            crop_size (int): size of the cropped image.
            normalize (tuple): mean and std for normalization.
        """
        ...
        if eval_resize_mode == "crop":
            crop_params = short_side_size
        elif eval_resize_mode == "squash":
            crop_params = crop_size, crop_size
        elif eval_resize_mode == "crop_full":
            crop_params = crop_size
        elif eval_resize_mode == "baboonland3":
            crop_params = 3 * crop_size # crop to a third in the center
        elif eval_resize_mode == "baboonland2":
            crop_params = 2 * crop_size # crop to a third in the center
        else:
            raise ValueError(f"Unknown eval resize mode {eval_resize_mode}")
        
        self.eval_transform = video_transforms.Compose([
            video_transforms.Resize(crop_params, interpolation='bilinear'),
        ...
        if self.aug_mode == "timo":
            self.v2_augs = v2.Compose([
                v2.RandomApply([v2.Grayscale(num_output_channels=3)], p=0.5), 
                v2.RandomApply([v2.ColorJitter(brightness=(0.5, 1.5), contrast=(0.7, 1.3), saturation=(0.7, 1.3), hue=None)], p=0.5),
            ])
        elif self.aug_mode == "timo2":
            self.v2_augs = v2.Compose([
                v2.RandomApply([v2.Grayscale(num_output_channels=3)], p=0.25), 
                v2.RandomApply([v2.ColorJitter(brightness=(0.5, 1.5), contrast=(0.7, 1.3), saturation=(0.7, 1.3), hue=None)], p=0.25),
                v2.RandomApply([v2.GaussianBlur(kernel_size=3, sigma=(0.1, 2.0))], p=0.25),
                v2.RandomApply([v2.RandomRotation(degrees=(-15, 15))], p=0.25)
            ])
        else:
            self.v2_augs = None
    ...
    def __call__(self, buffer):
        ...
        if self.eval_resize_mode in ["baboonland3", "baboonland2"]:
            video_transforms.CenterCrop((buffer[0].height // 2, buffer[0].width // 2))(buffer)
...
class EvalVideoTransform(object):
    ...
    def __init__(
        ...
        if resize_mode == "crop":
            ...
            self.spatial_resize = video_transforms.Resize(
                (short_side_size, short_side_size), interpolation='bilinear')
        else:
    ...
    def __call__(self, buffer):
        ...
        for i in range(num_views):
            ...
            if num_views == 1:
                start = (H - side_len) // 2 if H > W else (W - side_len) // 2
