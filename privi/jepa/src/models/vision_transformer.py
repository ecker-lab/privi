...
from privi.jepa.src.models.utils.patch_embed import PatchEmbed, PatchEmbed3D
from privi.jepa.src.models.utils.modules import Block
from privi.jepa.src.models.utils.pos_embs import get_2d_sincos_pos_embed, get_3d_sincos_pos_embed
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, masks=None):
        ...
        if masks is not None:
            #print(f"x shape before: {x.shape}, masks shape: {[m.shape for m in masks]}")
            x = apply_masks(x, masks)
            #print(f"x shape after: {x.shape}, masks shape: {[m.shape for m in masks]}")
